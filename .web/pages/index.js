...
export default function Component() {
  ...
  {state.home_state.followers.map((etwzyahg, lgsdzakk) => (
  <VStack key={lgsdzakk} sx={{"padding": "1em"}}>
  <HStack>
  <Avatar name={etwzyahg.follower_username} size={`sm`}/>
  <Text>
  {etwzyahg.follower_username}
</Text>
</HStack>
</VStack>
))}
  ...
  <Fragment>
  {state.home_state.tweets.map((bxpchlnr, axextkhi) => (
  <Grid key={axextkhi} sx={{"gridTemplateColumns": "1fr 5fr", "py": 4, "gap": 1, "borderBottom": "1px solid #ededed"}}>
  <VStack>
  <Avatar name={bxpchlnr.author} size={`sm`}/>
</VStack>
  <Box>
  ...
  {state.home_state.search_users.map((sspxxekd, gpsocgbs) => (
  <VStack key={gpsocgbs} sx={{"py": 2, "width": "100%"}}>
  <HStack sx={{"width": "100%"}}>
  <Avatar name={sspxxekd.username} size={`sm`}/>
  <Text>
  {sspxxekd.username}
</Text>
  <Spacer/>
  <Button onClick={(_e) => addEvents([Event("state.home_state.follow_user", {username:sspxxekd.username})], (_e), {})}>
  <AddIcon/>
</Button>
</HStack>
</VStack>
  ...
  {state.home_state.following.map((evsgnayv, ownmbsmh) => (
  <VStack key={ownmbsmh} sx={{"padding": "1em"}}>
  <HStack>
  <Avatar name={evsgnayv.followed_username} size={`sm`}/>
  <Text>
  {evsgnayv.followed_username}
</Text>
</HStack>
</VStack>
))}
