...
export default function Component() {
  ...
  {state.home_state.followers.map((wyocmbdr, sfbyrvew) => (
  <VStack key={sfbyrvew} sx={{"padding": "1em"}}>
  <HStack>
  <Avatar name={wyocmbdr.follower_username} size={`sm`}/>
  <Text>
  {wyocmbdr.follower_username}
</Text>
</HStack>
</VStack>
))}
  ...
  <Fragment>
  {state.home_state.tweets.map((zpsxgzst, njpbayyw) => (
  <Grid key={njpbayyw} sx={{"gridTemplateColumns": "1fr 5fr", "py": 4, "gap": 1, "borderBottom": "1px solid #ededed"}}>
  <VStack>
  <Avatar name={zpsxgzst.author} size={`sm`}/>
</VStack>
  <Box>
  ...
  {state.home_state.search_users.map((ncqhxcwv, loxbxvwn) => (
  <VStack key={loxbxvwn} sx={{"py": 2, "width": "100%"}}>
  <HStack sx={{"width": "100%"}}>
  <Avatar name={ncqhxcwv.username} size={`sm`}/>
  <Text>
  {ncqhxcwv.username}
</Text>
  <Spacer/>
  <Button onClick={(_e) => addEvents([Event("state.home_state.follow_user", {username:ncqhxcwv.username})], (_e), {})}>
  <AddIcon/>
</Button>
</HStack>
</VStack>
  ...
  {state.home_state.following.map((zsctnbhk, eqmqgjzy) => (
  <VStack key={eqmqgjzy} sx={{"padding": "1em"}}>
  <HStack>
  <Avatar name={zsctnbhk.followed_username} size={`sm`}/>
  <Text>
  {zsctnbhk.followed_username}
</Text>
</HStack>
</VStack>
))}
