...
export default function Component() {
  ...
  {state.home_state.followers.map((gtdfgxjz, xvoxcodc) => (
  <VStack key={xvoxcodc} sx={{"padding": "1em"}}>
  <HStack>
  <Avatar name={gtdfgxjz.follower_username} size={`sm`}/>
  <Text>
  {gtdfgxjz.follower_username}
</Text>
</HStack>
</VStack>
))}
  ...
  <Fragment>
  {state.home_state.tweets.map((isnxioqn, rrahgwjh) => (
  <Grid key={rrahgwjh} sx={{"gridTemplateColumns": "1fr 5fr", "py": 4, "gap": 1, "borderBottom": "1px solid #ededed"}}>
  <VStack>
  <Avatar name={isnxioqn.author} size={`sm`}/>
</VStack>
  <Box>
  ...
  {state.home_state.search_users.map((hcdggbwc, qhtanoya) => (
  <VStack key={qhtanoya} sx={{"py": 2, "width": "100%"}}>
  <HStack sx={{"width": "100%"}}>
  <Avatar name={hcdggbwc.username} size={`sm`}/>
  <Text>
  {hcdggbwc.username}
</Text>
  <Spacer/>
  <Button onClick={(_e) => addEvents([Event("state.home_state.follow_user", {username:hcdggbwc.username})], (_e), {})}>
  <AddIcon/>
</Button>
</HStack>
</VStack>
  ...
  {state.home_state.following.map((qsmudlku, wixjkfgz) => (
  <VStack key={wixjkfgz} sx={{"padding": "1em"}}>
  <HStack>
  <Avatar name={qsmudlku.followed_username} size={`sm`}/>
  <Text>
  {qsmudlku.followed_username}
</Text>
</HStack>
</VStack>
))}
